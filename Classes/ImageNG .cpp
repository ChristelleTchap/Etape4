...
ImageNG::ImageNG(int id,const char* nom,const Dimension& dim)
{
    #ifdef DEBUG
...
ImageNG::ImageNG(const ImageNG& autreImage)
{
    int i,j;
...
int ImageNG::getPixel(int x,int y)const
{
    ...
    {
        cout<<"Largeur grande"<<endl;
    }
    else
    {
        if(y<0 || y>dimension.getHauteur())
        {
           cout<<"Hauteur grande"<<endl; 
        }
        else
        {
            return matrice[x][y];
...
void ImageNG::setPixel(int x,int y,int val)
{
    if(val>255 || val<0)
    {
        cout<<"Gris incorrect"<<endl;
    }
    else
    {
        if(x<0 || x>dimension.getLargeur())
        {
           cout<<"Largeur grand"<<endl;
        }
        else
        {
            if(y<0 || y>dimension.getHauteur())
            {
                cout<<"Hauteur grand"<<endl;
            }
            else
            {
                matrice[x][y]=val;
...
void ImageNG::setBackground(int val) // mets tout les pixels de l`image au niveau gris
{
    ...
    if(val<0 || val>255) //val verifie si la valeur est en dehors de[0,255]
    {
        cout<<"Niveau gris incorrect"<<endl;
    }
    else
    {
...
void ImageNG::Dessine()  // appelle la méthode ViewImage() de la classe MyQT en lui passant *this en paramètre.
{
    MyQT::ViewImage(*this);
}
...
ImageNG operator+(const int val,const ImageNG& img)
{
